    ...
    virtual void    FrameResized(float width, float height)
                    {
                        ...
                        fBitmap = new BBitmap(r, 0, B_RGBA32);
                        if (!fBitmap->IsValid()) {
                            delete fBitmap;
                            fBitmap = NULL;
                        } else {
...
//      m_specific->m_redraw_flag = true;
    ...
    bool platform_support::load_img(unsigned idx, const char* file)
    {
        if (idx < max_images)
        {
            ...
            ret = path.InitCheck();
            if (ret < B_OK) {
                fprintf(stderr, "making path failed: %s\n", strerror(ret));
                return false;
            }
            ret = path.GetParent(&path);
            if (ret < B_OK) {
                fprintf(stderr, "getting parent folder failed: %s\n", strerror(ret));
                return false;
            }
            ret = path.Append(file);
            if (ret < B_OK) {
                fprintf(stderr, "appending image file name failed: %s\n", strerror(ret));
                return false;
            }
            BBitmap* transBitmap = BTranslationUtils::GetBitmap(path.Path());
