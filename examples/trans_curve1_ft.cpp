...
public:
    ...
    virtual void on_draw()
    {
        ...
        if(m_feng.load_font("timesi.ttf", 0, agg::glyph_ren_outline))
        {
            double x = 0.0;
            double y = 3.0;
            const char* p = text;
        ...
        ras.render_ctrl(sl, r, m_close);
        ras.render_ctrl(sl, r, m_preserve_x_scale);
        ras.render_ctrl(sl, r, m_fixed_len);
        ras.render_ctrl(sl, r, m_animate);
        ras.render_ctrl(sl, r, m_num_points);
        //--------------------------
